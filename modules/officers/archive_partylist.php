...
try {
    ...
    // Only admin can archive partylists
    if ($user_role != 1) {
        throw new Exception('You do not have permission to archive partylists. Only admins can archive partylists.');
    }
    
    // Check if partylist exists
    $stmt = $conn->prepare("SELECT id FROM partylist WHERE id = ?");
    if ($stmt === false) {
    ...
    // Archive the partylist and all associated officers in a transaction
    ...
    try {
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Partylist and associated officers archived successfully'
        ]);
