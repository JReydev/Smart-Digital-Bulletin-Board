...
try {
    ...
    // Only admin can restore officers
    if ($user_role != 1) {
        throw new Exception('You do not have permission to restore officers. Only admins can restore archived officers.');
    }
    
    // Check if officer exists and is archived
    $stmt = $conn->prepare("SELECT id FROM officers WHERE id = ? AND is_archived = 1");
    if ($stmt === false) {
    ...
    if (!$officer) {
        throw new Exception('Archived officer not found');
    }

    // Restore the officer
    $stmt = $conn->prepare("UPDATE officers SET is_archived = 0 WHERE id = ?");
    if ($stmt === false) {
        throw new Exception('MySQL prepare failed: ' . $conn->error);
    }
    ...
    if (!$stmt->execute()) {
        throw new Exception('Error restoring officer: ' . $stmt->error);
    }
    $stmt->close();
    
    echo json_encode([
        'success' => true,
        'message' => 'Officer restored successfully'
    ]);
