...
// Only admins can view archived faculty
$user_role = $_SESSION['role_id'] ?? 0;
if ($user_role != 1) {
    $_SESSION['error'] = 'You do not have permission to view archived faculty. Only admins can access this page.';
...
<div class="container">
    <div class="page-header">
        <h1><i class="fas fa-archive"></i> Archived Faculty</h1>
        <a href="faculty.php" class="back-button">
    ...
    $faculty = mysqli_query($conn, "SELECT f.*, m.file_path, u.username, u.is_active as user_active, u.profile_picture
                                   ...
                                   WHERE f.type = 'faculty' AND f.is_archived = 1
                                   ORDER BY f.lname ASC, f.fname ASC, f.mname ASC");
    
    if (mysqli_num_rows($faculty) == 0) {
        echo '<div class="empty-state">
                <i class="fas fa-archive"></i>
                <h2>No Archived Faculty</h2>
                <p>There are currently no archived faculty members.</p>
              </div>';
    } else {
        while ($row = mysqli_fetch_assoc($faculty)) {
            ...
            if (!empty($row['profile_picture']) && $row['profile_picture'] !== 'default-avatar.png') {
                ...
                $image_to_display = '../../images/default-avatar.jpg';
            }
            
            echo "<img src='{$image_to_display}' alt='{$display_name}' class='faculty-image' onerror=\"this.src='../../images/default-avatar.jpg'\">";
            
            echo "<div class='faculty-info'>";
            echo "<h3>{$display_name}<span class='archived-badge'>Archived</span></h3>";
            ...
            if (!empty($row['specialization'])) {
                ...
                echo "<strong>Specializations:</strong><br>";
            ...
            echo "<button type='button' class='restore-btn' onclick=\"showRestoreModal('{$row['id']}', '" . htmlspecialchars($display_name, ENT_QUOTES) . "')\">
                    <i class='fas fa-undo'></i> Restore
                  </button>";
            echo "</div>";
            
            echo "</div>"; // faculty-info
            echo "</div>"; // faculty-card
        }
    }
    ?>
</div>
...
<div class="modal" id="restoreModal">
    <div class="modal-content">
        <div class="modal-header">
            <h3><i class="fas fa-undo"></i> Restore Faculty Member</h3>
            <button type="button" class="close" onclick="closeRestoreModal()">&times;</button>
        </div>
        <div class="modal-body">
            <p>Are you sure you want to restore this faculty member?</p>
            <p class="faculty-name" id="facultyName"></p>
            <p class="info-text">
                <i class="fas fa-info-circle"></i> This faculty profile will be restored and will appear in the active faculty list.
            </p>
        </div>
...
function confirmRestore() {
    ...
    fetch('restore_faculty.php', {
        ...
        body: `faculty_id=${encodeURIComponent(restoreId)}`
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            closeRestoreModal();
            showSuccess(data.message || 'Faculty member restored successfully');
            setTimeout(() => location.reload(), 1000);
        } else {
            throw new Error(data.message || 'Failed to restore faculty member');
        }
    })
    .catch(error => {
        console.error('Error:', error);
        showError(error.message || 'Failed to restore faculty member. Please try again.');
