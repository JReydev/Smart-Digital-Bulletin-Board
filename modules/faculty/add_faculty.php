...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $media_id = NULL; // Will be updated if file is uploaded
    
    // Check if we need to create a user account for this faculty member
    $create_user_account = isset($_POST['create_user_account']) && $_POST['create_user_account'] === 'yes';
    $faculty_user_id = null;
    
    if ($create_user_account) {
        // Generate username from faculty name
        $username = strtolower(preg_replace('/[^a-zA-Z0-9]/', '', $fname . $lname));
        ...
        $default_password = password_hash('faculty123', PASSWORD_DEFAULT); // Default password
        $stmt = $conn->prepare("INSERT INTO users (username, first_name, middle_name, last_name, password, role_id, is_active) VALUES (?, ?, ?, ?, ?, 4, 1)");
        $stmt->bind_param("sssss", $username, $fname, $mname, $lname, $default_password);
        
        if ($stmt->execute()) {
            $faculty_user_id = $conn->insert_id;
        } else {
            $_SESSION['error'] = 'Failed to create user account for faculty member';
    ...
    if (isset($_FILES['faculty_image']) && $_FILES['faculty_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_extension, $allowed_extensions)) {
            ...
            if (move_uploaded_file($_FILES['faculty_image']['tmp_name'], $upload_path)) {
                ...
                // Insert into multimedia_content table
    ...
    // Use faculty user ID if created, otherwise use current user ID
    $faculty_record_user_id = $faculty_user_id ? $faculty_user_id : $user_id;
    
    $stmt = $conn->prepare("INSERT INTO faculty (prefix, fname, mname, lname, suffix, name, position, description, specialization, media_id, user_id, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'faculty')");
    $stmt->bind_param("sssssssssii", $prefix, $fname, $mname, $lname, $suffix, $name, $position, $description, $specialization, $media_id, $faculty_record_user_id);
    $stmt->execute();
    header("Location: faculty.php");
    exit;
}
...
<head>
    ...
    <style>
        * { 
        ...
        /* Unified page header is defined in header.php as .header */
        ...
        @media (max-width: 768px) {
            ...
            .form-group[style*="flex"] > div {
                width: 100% !important;
                flex: 1 1 100% !important;
                margin-bottom: 20px;
            }

            .form-group[style*="display: flex"] > div:last-child,
            .form-group[style*="flex"] > div:last-child {
                margin-bottom: 0;
            }
...
<div class="container">
    <form action="" method="POST" enctype="multipart/form-data">
        <div class="form-group">
            ...
            <input type="text" id="prefix" name="prefix" placeholder="e.g., Dr., Prof., Engr., Mr., Mrs., Ms.">
        </div>
        ...
        <div class="form-group" style="display: flex; gap: 15px;">
            ...
            <div style="flex: 1;">
                ...
                <input type="text" id="mname" name="mname" placeholder="Middle name or initial">
            </div>
            <div style="flex: 1;">
        ...
        <div class="form-group">
            ...
            <input type="text" id="suffix" name="suffix" placeholder="e.g., Jr., Sr., III, MIT, DIT, PhD">
        </div>
        ...
        <div class="form-group">
            ...
            <div class="description-container">
                ...
                <input type="hidden" id="position" name="position" value="" required>
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" placeholder="Add a brief description about this faculty member..."></textarea>
        </div>
        ...
        <div class="form-group">
            <label for="create_user_account">
                <input type="checkbox" id="create_user_account" name="create_user_account" value="yes" style="margin-right: 8px;">
                <i class="fas fa-user-plus"></i> Create User Account for Faculty Member
            </label>
            <p class="help-text">
                <i class="fas fa-info-circle"></i>
                Check this to create a login account for this faculty member. Default password will be 'faculty123'.
            </p>
        </div>
        ...
        <div class="buttons">
            <button type="submit" class="submit-btn"><i class="fas fa-plus"></i> Add Faculty</button>
